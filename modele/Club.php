...
class Club
{
    ...
    public function findWithStade(): array
    {
        // Clubs avec libellés du stade (nom/ville)
        $sql = "SELECT c.*, s.nom AS stadeNom, s.ville AS stadeVille
                FROM {$this->table} c
                LEFT JOIN stade s ON s.idStade = c.idStade
                ORDER BY c.nom";
        $stmt = $this->db()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);

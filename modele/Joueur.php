...
class Joueur
{
    ...
    public function findWithClub(): array
    {
        // Joueurs avec libellé du club
        $sql = "SELECT j.*, c.nom AS clubNom
                FROM {$this->table} j
                LEFT JOIN club c ON c.idClub = j.idClub
                ORDER BY j.nom";
        $stmt = $this->db()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    {
        // Joueurs présents dans un match via leurs clubs opposés
        $sql = "SELECT j.*, c.nom AS clubNom
                FROM {$this->table} j
                INNER JOIN oppose o ON o.idClub = j.idClub
                INNER JOIN matchrugby m ON m.idMatch = o.idMatch
                LEFT JOIN club c ON c.idClub = j.idClub
